...
namespace Fire_Detector.Dialog
{
    public partial class AutoFlyingDialog : Form
    {
        ...
        {
            this._gmap.SetPosition(37.3487547f, 126.74490070000002f);
        ...
        private void gmapBox_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (moved == System.Drawing.Point.Empty)
            {
                if (this._begin.IsEmpty || !this._end.IsEmpty)
                {
                    ...
                    path.Points.Add(this._gmap.Pixel2Coord(this._begin));

                    this._gmap.DrawMarker = true;
                    this._gmap.AddPath("begin", path);
                    this._gmap.UpdateGmap();
                }
                else
                {
                    ...
                    var polygon = new OYOGmapPolygon(3, Color.FromArgb(64, Color.Blue), Color.Transparent);
                    polygon.Points.Add(this._gmap.Pixel2Coord(this._begin));
                    polygon.Points.Add(this._gmap.Pixel2Coord(left_bot));
                    polygon.Points.Add(this._gmap.Pixel2Coord(this._end));
                    polygon.Points.Add(this._gmap.Pixel2Coord(right_top));
